...
const tickerConfigs: TickerMockConfig[] = [
  { ticker: "MU", spot: 1190.02, changePercent: 13.5, baseIv: 0.29, strikeMin: 800, strikeMax: 1450, strikeStep: 50, callWallStrike: 1200, putWallStrike: 1000, maxPainBiasStrike: 850, trend: "neutral" },
  { ticker: "AMD", spot: 164.4, changePercent: 3.2, baseIv: 0.34, strikeMin: 110, strikeMax: 220, strikeStep: 10, callWallStrike: 160, putWallStrike: 145, maxPainBiasStrike: 155, callSkew: 1.18, trend: "bullish" },
  { ticker: "NVDA", spot: 154.8, changePercent: 5.8, baseIv: 0.38, strikeMin: 100, strikeMax: 220, strikeStep: 10, callWallStrike: 150, putWallStrike: 135, maxPainBiasStrike: 150, callSkew: 1.35, putSkew: 0.82, volumeMultiplier: 1.18, trend: "bullish" },
  { ticker: "TSLA", spot: 322.3, changePercent: -2.1, baseIv: 0.58, strikeMin: 220, strikeMax: 460, strikeStep: 20, callWallStrike: 380, putWallStrike: 280, maxPainBiasStrike: 300, callSkew: 1.08, putSkew: 1.18, volumeMultiplier: 1.08, trend: "volatile" },
  { ticker: "AAPL", spot: 214.65, changePercent: 0.8, baseIv: 0.22, strikeMin: 170, strikeMax: 260, strikeStep: 10, callWallStrike: 230, putWallStrike: 200, maxPainBiasStrike: 210, callSkew: 0.95, putSkew: 0.95, volumeMultiplier: 1.22, trend: "stable" },
  { ticker: "HOOD", spot: 87.25, changePercent: 8.9, baseIv: 0.74, strikeMin: 50, strikeMax: 130, strikeStep: 5, callWallStrike: 105, putWallStrike: 70, maxPainBiasStrike: 80, callSkew: 1.22, putSkew: 1.12, volumeMultiplier: 0.76, trend: "risky" },
];
...
export function getUniverseTickers(): string[] {
  return mockUniverse.map((item) => item.quote.ticker);
}

export function getTickerProjection(ticker: string): RawOptionProjectionInput | null {
  const projection = mockUniverse.find((item) => item.quote.ticker === ticker);

  if (!projection) {
    return null;
  }

  return withMockDataSource(projection);
}

function withMockDataSource(projection: RawOptionProjectionInput): RawOptionProjectionInput {
  return {
    ...projection,
    dataSource: {
      marketData: "mock",
